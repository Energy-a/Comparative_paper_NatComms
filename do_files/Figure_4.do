...
/*------------------------------------------------------------------------------------------------------------------------------------------
 ...
 This do-file: 
   ...
   2) Compute the cooling gap index
   3) Data for Figure 4
...
use "`data'", clear
merge m:1 year country state3 using "`fut_pop'"
keep if _merge == 3
drop _merge
...
collapse (count) hh_state_fut_`ssp' [pw=weight_`ssp'] , by(year country state3) /* This gives the weighted sum of all families */
...
collapse (mean) ac ac_fut_* (count) hh_state [pw=weight] , by(year country state3)
...
merge 1:1 year country state3 using "`cdd_fut'"
keep if _merge == 3
drop _merge
...
merge 1:1 year country state3 using "`fut_pop_`ssp''"
keep if _merge == 3
drop _merge
...
*** 6) Compute number of HH in each quadrants
** Current level
* Q1
...
* Q2 - Current
egen TOT_N_HH_q2 = sum(n_with_ac) if ac_ratio > 1 & exposure_ratio < 1 // top-left: low-exposure, high-adoption
...
** 9) Number of HHs with at least one AC to compare with IEA AC units numbers ***
...
** Save
save "output_data_files\projections\cdd_pop_exposure_4countries.dta", replace
